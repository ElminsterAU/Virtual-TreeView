...
type
  ...
  private
	...
	{>>>}
    FPopupIndex,                      // column on which the popup menu was opened
	{<<<}
  ...
  public
	...
	{>>>}
    property PopupIndex: TColumnIndex read FPopupIndex;
	{<<<}
...
begin
  ...
  begin
    ...
    {>>>}
      if Tree.UpdateCount > 0 then
        Tree.DoStateChange([tsSortPending])
      else
    {<<<}
...
{>>>}
procedure TVTHeader.SortTree;

// sorts the tree according to sort column and sort direction
...
begin
  ...
  begin
    ...
    begin
      {>>>}
      if not Tree.DoHeaderDropped(DragIndex, DropTarget) then begin
      {<<<}
...
begin
  Result := (Column > NoColumn) and ([coResizable, coVisible{>>>},coFiller{<<<}] * Items[Column].Options = [coResizable, coVisible]);
    ...
    begin
      ...
      begin
        ...
        TreeViewControl.DoStateChange([], [tsScrollPending, tsScrolling]);
			  {>>>}
        PointForIndex := P;
        Inc(PointForIndex.Y, Header.Height);
        FPopupIndex := ColumnFromPosition(PointForIndex);
			  {<<<}
...
begin
  ...
  {>>>}
  if (Result <> TreeViewControl.ClientWidth) then
    if LastColumn > NoColumn then
      with Items[LastColumn] do
        if coFiller in FOptions then begin
          ...
          if (Result >= TreeViewControl.ClientWidth) then
            ...
            FWidth := TreeViewControl.ClientWidth - Result;
            Result := TreeViewControl.ClientWidth;
          end;
        end;
  {<<<}
end;
